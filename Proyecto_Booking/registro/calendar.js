...
import { HOLD_MINUTES, MS_PER_DAY } from './config.js';
...
export function setUpRealtimeAvailability() {
  ...
  state.reservasUnsub = db
    .collection('reservas')
    .where('propertyId', '==', getCurrentPropertyId())
    .onSnapshot(
...
export function validateAndGoToStep2() {
  ...
  const adults = adultsInput ? parseInt(adultsInput.value || '0', 10) || 0 : 0;
  const children = childrenInput ? parseInt(childrenInput.value || '0', 10) || 0 : 0;

    if (adults < 1 || adults > 4) {
    alert('⚠️ El número de adultos debe estar entre 1 y 4.');
    return false;
  }
  if (children < 0 || children > 4) {
    alert('⚠️ El número de niños debe estar entre 0 y 4.');
    return false;
  }
  if (adults + children > 4) {
    alert('⚠️ Solo se permiten un máximo de 4 personas en total (adultos + niños).');
    return false;
  }
