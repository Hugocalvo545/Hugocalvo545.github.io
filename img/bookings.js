...
export function calculatePrice() {
  ...
  if (!state.bookCheckInDate || !state.bookCheckOutDate || !numAdults) return;
  ...
  form.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      const bookingDataRaw = {
        ...
        propertyId: 'atico-jerez',
        propertyName: 'Ático Dúplex en Jerez',
        reservaId,
        name: u.name || '',
        surname: u.surname || '',
        email: u.email || '',
      ...
      try{
        if(CONFIG.GOOGLE_SHEETS_URL){
          const formData=new FormData(); formData.append('payload',JSON.stringify(bookingData));
          await fetch(CONFIG.GOOGLE_SHEETS_URL,{method:'POST',body:formData,mode:'no-cors'});
        }
      }catch(_){}
